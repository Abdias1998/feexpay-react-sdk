...
export const CardBankPay: React.FC<Props> = ({ onChoice , change_send_signal}) => {
  ...
  React.useEffect(() => {
   async function sendCardRequest() {
       if (state.numero_card !== "" && state.name_card !== "" && state.email_card !== "" && state.title_card !== "" && state.address_card !== "" && state.locality_card !== "" && state.last_name_card !== "" && state.country_card !== "") {
        await axios.post(`http://192.168.0.122:4005/api/transactions/card/inittransact`,{
    ...
    sendCardRequest()
  }, [state.numero_card,state.name_card,state.email_card,state.title_card,state.address_card,state.locality_card,state.last_name_card,state.country_card,send_signal])
  return (
    <>
        ...
        <div className="choice_operator_card">
          ...
          <div className="choice_operator_img">
            <input type="radio" name="operator_choice" id="" onChange={()=>changereseauVISAValue()} />
            <img
              className="img_mtn"
            ...
            />
            <input type="radio" name="operator_choice" id="" onChange={()=>changereseauMASTERCARDValue()} />
       ...
       {iFrame && (
            <iframe src={urlPay} height="500" width="100%" style={{border:"none"}} name="demo"></iframe>
         ) 
       }
    </>
